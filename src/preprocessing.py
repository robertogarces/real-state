...
class Preprocessing(FlowSpec):
    ...
    def transform_functions(self):
        self.transform_price_log = FunctionTransformer(
            transform_price_log, validate=False
        )
        self.transform_area_units = FunctionTransformer(
            transform_area_units, validate=False
        )
        self.categorize_bedrooms = FunctionTransformer(
            categorize_bedrooms, validate=False
        )
        self.categorize_bathrooms = FunctionTransformer(
            categorize_bathrooms, validate=False
        )
        self.categorize_yearBuilt = FunctionTransformer(
            categorize_yearBuilt, validate=False
        )
        self.remove_garageSpaces_outliers = FunctionTransformer(
            remove_garageSpaces_outliers, validate=False
        )
        ...
        self.city_median_price = FunctionTransformer(
            calculate_statistic,
            validate=False,
            kw_args={"feature": "city", "statistic": "median"},
        )
        self.city_mean_price = FunctionTransformer(
            calculate_statistic,
            validate=False,
            kw_args={"feature": "city", "statistic": "mean"},
        )
        self.county_median_price = FunctionTransformer(
            calculate_statistic,
            validate=False,
            kw_args={"feature": "county", "statistic": "median"},
        )
        self.county_mean_price = FunctionTransformer(
            calculate_statistic,
            validate=False,
            kw_args={"feature": "county", "statistic": "mean"},
        )
        self._5_knn_median_price = FunctionTransformer(
            knn_property_price,
            validate=False,
            kw_args={"n_neighbors": 5, "statistic": "median"},
        )
        self._5_knn_mean_price = FunctionTransformer(
            knn_property_price,
            validate=False,
            kw_args={"n_neighbors": 5, "statistic": "mean"},
        )
        self._25_knn_median_price = FunctionTransformer(
            knn_property_price,
            validate=False,
            kw_args={"n_neighbors": 25, "statistic": "median"},
        )
        self._25_knn_mean_price = FunctionTransformer(
            knn_property_price,
            validate=False,
            kw_args={"n_neighbors": 25, "statistic": "mean"},
        )
        self.encoding = FunctionTransformer(
            encode_categorical_variables, validate=False
        )
        self.drop_features = FunctionTransformer(
            drop_features, validate=False, kw_args={"target": self.target}
        )
    ...
    def fit_preprocessing_pipeline(self):
        ...
        self.train_transformed.to_parquet(
            f"{PROCESSED_DATA_PATH}/transformed_dataset.parquet", index=False
        )
